...
def validatekey(api_key: str = Header(...)):
    if api_key == os.getenv("API_USER_KEY"):
        return "user"
    elif api_key == os.getenv("API_ADMIN_KEY"):
        return "admin"
    else:
...
@app.get("/questions")
def questions(session_id: str | None = Header(default=None) , role : str = Depends(validatekey)):
...
def SaveandFilter(data: Answers,role : str = Depends(validatekey)):
    if role in ("user", "admin"):
        botQuestions = db["botQuestion"]
...
@app.post("/rapidfuzzy")
def locationcheck(data: fuzzycheck ,  role : str = Depends(validatekey)):
...
@app.post("/otpgen")
def otpgenrator(data: validateEmailPhone, role: str = Depends(validatekey)):
    if role in ("admin", "user"):
...
@app.get("/validateOtp")
def validateOtp(session_id: str | None = Header(default=None),otp: str | None = Header(default=None),role: str = Depends(validatekey)):
    if role in ("admin","user"):
        validate = False
